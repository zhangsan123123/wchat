		...
		<table>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$repair): $mod = ($i % 2 );++$i;?><tr>
						...
						<td><?php echo (time_format($repair["deadline"])); ?></td>
						<td><?php echo (time_format($repair["create_time"])); ?></td>
						<td><?php echo (getstatus($repair["status"])); ?></td>
						<td>
							<?php if($repair["status"] == 1): ?><a href="<?php echo U('Active/status1?id='.$repair['id']);?>">审核通过</a>
								<a href="<?php echo U('Active/status2?id='.$repair['id']);?>">审核不通过</a>
								<?php else: endif; ?>
								<a class="btn" href="<?php echo U('del?id='.$repair['id']);?>">删 除</a>
