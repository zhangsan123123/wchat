...
<div class="main">
    ...
    <div class="main">
        <div class="container-fluid" id="content">
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$estate): $mod = ($i % 2 );++$i;?><div class="row noticeList">
                    <a href="<?php echo U('estate/detail?id='.$estate['id']);?>">
                        <div class="col-xs-2">
                            <img class="noticeImg" src="<?php echo (get_cover($estate["cover_id"],'path')); ?>" />
                        </div>
                        <form>
                            <input type="hidden" name="<?php echo ($estate["id"]); ?>">
                        </form>
                        <div class="col-xs-10">
                            <p class="title"><?php echo ($estate["title"]); ?></p>
                            <p class="intro"><?php echo ($estate["description"]); ?></p>
                            <p class="info">浏览: <?php echo ($estate["view"]); ?> <span class="pull-right"><?php echo (date('Y-m-d H:i:s',$estate["create_time"])); ?></span> </p>
                        </div>
                    </a>
    ...
    <script type="application/javascript">
        ...
        $('.ajax-get-self').click(function(){
            ...
            $.get('<?php echo U();?>',{'p':p+1},function(data){
                if(data){
                    var html = '';
                    $(data).each(function(i,e){
                        html += ' <div class="row noticeList">\
                                    <a href="<?php echo U('estate/detail','',false);?>/id/'+ e.id+'">\
                                            <div class="col-xs-2">\
